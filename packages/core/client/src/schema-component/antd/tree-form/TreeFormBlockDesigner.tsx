...
export const TreeFormBlockDesigner = () => {
  ...
  const targetFieldSchema = fieldSchema?.properties?.tree;
  const collctionName = targetFieldSchema?.['x-decorator-props']?.collection;

  const { getCollectionField, getCollection } = useCollectionManager();
  const collection = getCollection(collctionName);
  const { name, title, sortable } = collection;
  ...
  const defaultSort = targetFieldSchema?.['x-decorator-props']?.params?.sort || [];
  const defaultResource = targetFieldSchema?.['x-decorator-props']?.resource;
  const supportTemplate = !targetFieldSchema?.['x-decorator-props']?.disableTemplate;
